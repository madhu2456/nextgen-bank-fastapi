    ...
    async def validate_dependencies(self, service_name:str, depends_on: list[str]) -> None:
        ...
        for dep in depends_on:
            if dep not in self._services:
                raise ValueError(f"Dependency {dep} is not registered for service {service_name}")

    async def add_service(self, service_name:str, check_function: Callable[[], Awaitable[bool]], timeout: float=5.0,
                          retry_delay: float = 1.0, max_retries: int = 3, depends_on: list[str] | None = None)-> None:
        ...
        try:
            ...
            if not workers:
                ...
                try:
                    ...
                    logger.warning("No celery workers found, but RabbitMQ reachable")
        ...
        for attempt in range(max_retries):
            ...
            try:
                async with asyncio.timeout(timeout):
                    ...
                    if is_healthy:
                        async with self._lock:
                            ...
                            if attempt > 0:
                                logger.info(
                                    f"Service {service_name} recovered {metrics['attempts']} attempts"
                                )
                ...
                if attempt == max_retries - 1:
                    logger.warning(
                        f"Health check timeout for {service_name} after {max_retries - 1} retries"
                    )
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"Health check failed after {service_name}: {e}"
                    )

            metrics["total_delay"] = metrics["total_delay"] + retry_delay
